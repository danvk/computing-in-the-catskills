...
for name, lat, lng in [('lo', low_lat, low_lng), ('mid', mid_lat, mid_lng), ('hi', hi_lat, hi_lng)]:
...
# Usually within 0.3%
for a, b in zip(random.choices(parking_nodes, k=40), random.choices(parking_nodes, k=40)):
    lat1 = a['lat']
    lng1 = a['lon']
    lat2 = b['lat']
    lng2 = b['lon']
    ...
    delta = d_true - d_approx

    print(f'{d_true:.0f} vs {d_approx:.0f} ∆={abs(delta):.0f} {abs(delta)/d_true*100:.1f}%')
